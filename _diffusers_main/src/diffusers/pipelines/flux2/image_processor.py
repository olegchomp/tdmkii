...
class Flux2ImageProcessor(VaeImageProcessor):
    ...
    def check_image_input(
        image: PIL.Image.Image | torch.Tensor, max_aspect_ratio: int = 8, min_side_length: int = 64, max_area: int = 1024 * 1024
    ) -> PIL.Image.Image | torch.Tensor:
        """
        Check if image meets minimum size and aspect ratio requirements.

        Args:
            image: PIL Image or torch.Tensor (C,H,W) / (B,C,H,W) to validate
        ...
        if not isinstance(image, PIL.Image.Image):
            raise ValueError(f"Image must be a PIL.Image.Image or torch.Tensor, got {type(image)}")
