        ...
        if image is not None:
            ...
            for img in image:
                if isinstance(img, torch.Tensor):
                    # Tensor input (e.g. GPU→GPU from TouchDesigner): get dims from shape
                    if img.dim() == 4:
                        image_height, image_width = int(img.shape[2]), int(img.shape[3])
                    else:
                        image_height, image_width = int(img.shape[1]), int(img.shape[2])
                    if image_width * image_height > 1024 * 1024:
                        ...
                        img = torch.nn.functional.interpolate(
                            img.float() if img.dim() == 4 else img.unsqueeze(0).float(),
                            size=(new_h, new_w),
                            mode="bilinear",
                            align_corners=False,
                        ).to(img.dtype)
