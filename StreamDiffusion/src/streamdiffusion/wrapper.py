...
class StreamDiffusionWrapper:
    ...
    def _load_model(
        ...
        # Load and merge LoRA weights (fully fused into UNet + CLIP, then unloaded)
        ...
        if lora_adapters_to_merge:
            try:
                for adapter_name, scale in zip(lora_adapters_to_merge, lora_scales_to_merge):
                    logger.info(f"Fusing LoRA: {adapter_name} with scale {scale}")
                    stream.pipe.fuse_lora(lora_scale=scale, adapter_names=[adapter_name])

                stream.pipe.unload_lora_weights()
                logger.info("Successfully fused and unloaded all LoRAs")
            except Exception as e:
                logger.error(f"LoRA fuse failed: {e}")
                logger.warning("Continuing without LoRA merging")
                try:
                    stream.pipe.unload_lora_weights()
                except:
                    pass
