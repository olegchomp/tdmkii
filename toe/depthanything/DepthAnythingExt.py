...
class DepthAnythingExt:
	def __init__(self, ownerComp):
		...
		try:
			...
			self.get_dimensions(self.engine, self.context)
	...
	def get_dimensions(self, engine, context=None):
		"""Get input dimensions; support TensorRT 8 (num_bindings) and TensorRT 10+ (num_io_tensors)."""
		shapes = []
		if hasattr(engine, "num_io_tensors"):
			for idx in range(engine.num_io_tensors):
				name = engine.get_tensor_name(idx)
				if context is not None and engine.get_tensor_mode(name) == trt.TensorIOMode.INPUT:
					shape = context.get_tensor_shape(name)
					shapes.append(shape)
					break
			if not shapes:
				name = engine.get_tensor_name(0)
				shape = context.get_tensor_shape(name) if context else []
				shapes.append(shape)
		else:
			for binding in range(engine.num_bindings):
				shape = engine.get_binding_shape(binding)
				shapes.append(shape)
		dimensions = list(shapes[0][-2:]) if shapes else [518, 518]
	...
	def infer(self, img, output):
		"""Run inference on TensorRT engine; support TRT 8 (execute_async_v2) and TRT 10+ (set_tensor_address + execute_async_v3)."""
		cuda_stream = self.stream.cuda_stream
		if hasattr(self.context, "execute_async_v3"):
			self.context.set_tensor_address("input", img.data_ptr())
			...
			self.context.execute_async_v2(bindings=self.bindings, stream_handle=cuda_stream)
